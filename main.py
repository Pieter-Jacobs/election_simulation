...
def main(cfg: DictConfig):
    """Run the election with two polls and save the seat distribution, strategic voting percentage, happiness and vote directions"""
    for run in range(cfg.n_runs):
        ...
        for poll_nr in range(cfg.n_polls):
            ...
            write_string_to_file(string=f"{results}", path=hydra.utils.get_original_cwd() + os.sep + "data" + os.sep
                                 + "election_results" + os.sep + cfg.polls.name + "_swing_" + str(cfg.upper_swing) + "_run_" + str(run) + '_poll_' + cfg.polls.name + '_' + str(poll_nr))
            write_string_to_file(string=f"{percentage_strategic}", path=hydra.utils.get_original_cwd() + os.sep + "data" + os.sep
                                 + "strategic_voting_stats" + os.sep + cfg.polls.name + "_swing_" + str(cfg.upper_swing) + "_run_" + str(run) + '_poll_' + cfg.polls.name + '_' + str(poll_nr))
            write_string_to_file(string=f"{happiness}", path=hydra.utils.get_original_cwd() + os.sep + "data" + os.sep
                                 + "happiness" + os.sep + cfg.polls.name + "_swing_" + str(cfg.upper_swing) + "_run_" + str(run) + '_poll_' + cfg.polls.name + '_' + str(poll_nr))
            write_matrix_to_file(matrix=vote_matrix, path=hydra.utils.get_original_cwd(
            ) + os.sep + "data" + os.sep + "voter_matrices" + os.sep + cfg.polls.name + "_swing_" + str(cfg.upper_swing) + "_run_" + str(run) + '_poll_' + cfg.polls.name + '_' + str(poll_nr))
