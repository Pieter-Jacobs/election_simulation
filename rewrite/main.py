    ...
    for run in range(cfg.n_runs):
        ...
        for poll_nr in range(cfg.n_polls):
            ...
            write_string_to_file(string=f"{percentage_strategic}", path=hydra.utils.get_original_cwd() + os.sep + "data" + os.sep
                                + "strategic_voting_stats" + os.sep + cfg.polls.name + "_swing_" + str(cfg.upper_swing) + "_run_" + str(run) + str(run) + '_poll_' + str(poll_nr))
            write_matrix_to_file(matrix=vote_matrix, path=hydra.utils.get_original_cwd(
            ) + os.sep + "data" + os.sep + "voter_matrices" + os.sep + cfg.polls.name + "_swing_" + str(cfg.upper_swing) + "_run_" + str(run) + str(run) + '_poll_' + str(poll_nr))
